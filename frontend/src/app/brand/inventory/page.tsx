...
export default function BrandInventoryPage() {
    ...
    useEffect(() => {
        ...
        fetch('http://127.0.0.1:8000/api/brand/inventory/', {
    ...
    const handleBulkRestock = async () => {
        ...
        const requests = selectedItems.map(item => {
            ...
            return fetch('http://127.0.0.1:8000/api/factory/jobs/', {
                method: 'POST',
                headers: {
    ...
    const handleUpdateProduct = async () => {
        ...
        try {
            const res = await fetch(`http://127.0.0.1:8000/api/brand/products/${editItem.product}/`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`
