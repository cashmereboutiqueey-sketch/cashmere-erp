...
export default function CollectionsPage() {
    ...
    const handleSave = async () => {
        ...
        const url = formData.id
            ? `http://127.0.0.1:8000/api/brand/categories/${formData.id}/`
            : 'http://127.0.0.1:8000/api/brand/categories/';
    ...
    const handlePushToShopify = async (category: Category) => {
        ...
        try {
            const res = await fetch('http://127.0.0.1:8000/api/brand/shopify/push_collection/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
