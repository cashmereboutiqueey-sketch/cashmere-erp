...
export default function BrandFinancePage() {
    ...
    const fetchData = async () => {
        try {
            const [txRes, trRes, metricsRes, analyticsRes] = await Promise.all([
                fetch('http://localhost:8000/api/finance/transactions/?module=BRAND'),
                fetch('http://localhost:8000/api/finance/treasury/'),
                fetch('http://localhost:8000/api/finance/metrics/brand/'),
                fetch('http://localhost:8000/api/brand/analytics/dashboard/') // New Endpoint
            ]);
    ...
    const handleTransfer = async () => {
        ...
        try {
            const res = await fetch('http://localhost:8000/api/finance/treasury/transfer_to_main/', {
                method: 'POST',
