    ...
    def payroll(self, request):
        ...
        for worker in workers:
            total_hours = WorkerAttendance.objects.filter(
                worker=worker, date__range=[start_date, end_date]
        ...
        return Response({'period': {'start_date': start_date, 'end_date': end_date}, 'payroll': payroll_data})
...
class RawMaterialViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if float(initial_stock) > 0:
            ...
            try:
                ...
                logger.error(f"Error creating initial stock purchase for {material.name}: {e}", exc_info=True)
        else:
            serializer.save()
    ...
    def import_csv(self, request):
        ...
        try:
            ...
            if result['success']:
                return Response({'success': True, 'created': result['created'], 'updated': result['updated'], 'warnings': result['warnings']})
            return Response({'success': False, 'errors': result['errors'], 'warnings': result['warnings']}, status=400)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if request.data.get('apply_to_variants'):
            try:
                ...
                logger.error(f"BOM variant propagation failed: {e}", exc_info=True)
...
class ProductionJobViewSet(viewsets.ModelViewSet):
    ...
    def velocity(self, request):
        ...
        return Response([{'date': d['date'], 'quantity': d['quantity']} for d in data if d['date']])
    ...
    def dashboard_stats(self, request):
        ...
        pending_jobs = ProductionJob.objects.filter(status=ProductionJob.JobStatus.PENDING).count()
        low_stock_materials = RawMaterial.objects.filter(current_stock__lt=F('minimum_stock_level')).count()
        this_month = timezone.now().date().replace(day=1)
        completed_qty_this_month = ProductionJob.objects.filter(
            status=ProductionJob.JobStatus.COMPLETED, end_date__gte=this_month
        ).aggregate(total=Sum('quantity'))['total'] or 0
        last_30_days = timezone.now().date() - datetime.timedelta(days=30)
        velocity_data = ProductionJob.objects.filter(
            status=ProductionJob.JobStatus.COMPLETED, end_date__gte=last_30_days
        ).values('end_date').annotate(quantity=Sum('quantity')).order_by('end_date')
        return Response({
            'kpis': {
            ...
            'production_velocity': [
                ...
                for d in velocity_data if d['end_date']
            ]
        })
